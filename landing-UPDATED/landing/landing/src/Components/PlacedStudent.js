...
useEffect(() => {
  axios
    ...
    .then((response) =>
    {
      const data = response.data.map((placed)=> {
        // console.log(placed.dateofPlaced)
        return {
          "studentId": placed.studentId,
  "studentName": placed.studentName,
  "courseStudied": placed.courseStudied,
  "courseDuration": placed.courseDuration,
  "package": placed.package,
  "dateofPlaced": placed.dateofPlaced.split('T')[0],
  "companyPlaced": placed.companyPlaced,
  "contactMail": placed.contactMail
        }
      })
...
// Add New Student
const addStudent = () => {
  console.log(newStudent.dateOfPlaced)
const postData =   {
    "studentId": 0,
    "studentName": newStudent.studentName,
    "courseStudied": newStudent.courseStudied,
    "courseDuration": newStudent.courseStudied,
    "package": parseInt(newStudent.package),
    "dateofPlaced": convertToDateTime(newStudent.dateOfPlaced),
    "companyPlaced": newStudent.companyPlaced,
    "contactMail": newStudent.contactMail
  }
  axios
    .post("https://localhost:7256/api/RecentlyPlacedStudents", postData)
    .then((response) => {
            ...
            {students.length ? (
              students.map((student) => (
                <TableRow key={student.studentId}>
                  ...
                  <TableCell>{student.dateofPlaced}</TableCell>
